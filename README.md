...
- 多会话指挥台：在一个 GUI 中查看、筛选和切换多个 Ai Pair 会话
- 一键启动双栏终端：左侧 `Claude Code`，右侧 `Codex CLI`
- 使用应用自带的 WezTerm 配置，不修改用户全局配置
- 自动记录会话目录、运行状态和最近一次绑定信息，减少重复输入和手动恢复
- 支持手动刷新后发现 WezTerm 工作区并重连断开的会话
- 支持从左到右、从右到左发送终端上下文
- GUI 内可切换 Claude / Codex 启动模式
- 支持 Launch Profile 模板套用与保存当前启动配置
- 新建会话时可选 `worktree` 目录隔离
...
- 自动编排运行状态只保存在当前应用生命周期内。
- 当前版本不支持在应用重启后恢复正在运行的自动编排上下文；重启后如需继续，请重新启动会话并重新开启自动模式。
