...
┌─────────────────────────────────────────────┐
│  Phase 0: 需求注入                            │
│  GUI → Phase 1 执行器（Default Mode，不开 Plan） │
│  执行器收到用户需求文本                         │
└───────────────────┬─────────────────────────┘
                    │
                    ▼
┌─────────────────────────────────────────────┐
│  Phase 1: 项目调研                            │
│  Phase 1 执行器调度 researcher + planner       │
...
┌─────────────────────────────────────────────┐
│  Phase 3: 任务执行                            │
│  Phase 3 执行器调度 6 个 SubAgent 并行/串行执行  │
│  按 task.md 清单逐项推进                       │
│  实时更新 task.md 进度标记                      │
└───────────────────┬─────────────────────────┘
                    │ 执行完毕
...
┌─────────────────────────────────────────────┐
│  Phase 4: 复核验收                            │
│  Phase 4 执行器调用 reviewer + tester SubAgent │
│  逐项复核 task.md 完成情况                      │
│  未通过项回退 → Phase 3 局部重试               │
...
### 1.3 Phase 执行器矩阵（规划说明）

- `Phase 1-4` 允许分别选择 `Claude` 或 `Codex` 作为负责人。
- `Phase 0` 不单独配置，固定跟随 `Phase 1` 执行器。
- 默认矩阵保持现有行为：`P1=Claude`、`P2=Claude`、`P3=Codex`、`P4=Claude`。
...
2. 点击"启动自动编排"按钮
3. GUI 构建 Bootstrap 提示词，发送到 Phase 1 执行器对应的 pane
4. Phase 1 执行器启动模式: **Default Mode**（非 Plan Mode），权限已预授权
5. 另一侧执行器保持自动模式可用（免确认），避免后续阶段切换时中断
...
Phase 1 执行器以 Default Mode 运行，调用 `researcher` 和 `planner` SubAgent 对目标项目进行全面调研，最终在项目根目录生成 `task.md` 文件。
...
task.md 生成后，Phase 1 执行器输出结构化数据包通知 GUI（示例以 Claude 为例）:
...
Phase 2 执行器切换到 Plan Mode，读取 task.md，调用 6 个 SubAgent 进行协同规划，产出精细化的执行计划。

### 4.2 Phase 2 触发提示词

用户确认后，GUI 向 Phase 2 执行器发送:

> 当前实现说明：
> - 继续复用现有 `AutoCollaborationCoordinator`
> - `stage_id` 保持从 `1` 开始，不再使用 `0`
> - 通过新增 `phase` 字段区分 Phase 1/2/3/4
> - 新增 `handoff_brief` 作为阶段交接摘要字段，`codex_brief` 仅作为兼容别名
...
Phase 2 执行器汇总六角色输出后，更新 task.md 状态为 `PLANNED`，并输出数据包（示例以 Claude 为例）:
...
Phase 3 执行器调度 6 个 SubAgent 按 task.md 清单逐项执行，实时更新进度标记。

### 5.2 Phase 3 触发提示词

用户确认后，GUI 向 Phase 3 执行器发送执行指令。
...
#### 执行器提示词（兼容 Codex/Claude）
```
你现在是执行者。上游会分阶段向你发送具体的代码执行指令。
每次收到指令后:
1. 执行代码修改
2. 运行验证命令
...
每个阶段完成后，Phase 3 执行器输出（示例以 Codex 为例）:
...
Phase 4 执行器调用 `reviewer`、`tester`、`debugger` SubAgent 对执行结果进行全面复核，逐项验证 task.md 中的完成状态。

### 6.2 Phase 4 触发条件

- Phase 3 所有阶段的 `execution_report` 收集完毕
- Phase 4 执行器自动进入复核，无需用户触发
...
> 说明：以下示例以 Claude 为例，实际应以 Phase 4 执行器为准。
...
> 说明：以下模板以 Claude 为示例。若 Phase 执行器设置为 Codex，请将身份描述与 role 约束替换为 Codex，对应输出结构化数据包保持不变。
...
| task.md 格式异常 | 通知 Phase 1 执行器重新生成 | → Phase1_GeneratingTaskMd |
...
- [x] **O8**: 实现六角色完整协同规划
- [x] **O18**: Phase 执行器矩阵与动态路由
  - 对齐说明：`Phase 1-4` 已支持在 GUI 中分别选择 `Claude` / `Codex`，`Phase 0` 跟随 `Phase 1` 执行器，Coordinator 已按 Phase 路由动态分发与校验结构化数据包。
- [x] **O9**: 实现阶段内并行执行
  - 对齐说明：当前实现为“单 pane 协调、执行器内部 SubAgent 并行”，`parallel_group` 已进入协议、提示词和界面展示，不扩展为 Launcher 级多 pane fan-out。
...
- [x] **O14**: task.md 变更历史可视化
  - 对齐说明：已新增 task.md revision 持久化与 Automation 页面历史列表，按内容哈希记录状态、阶段、执行器与摘要。
- [x] **O15**: SubAgent 执行并行度自适应
  - 对齐说明：已新增 `Conservative / Balanced / Aggressive / Auto` 并发策略和最大并发数配置，配置进入 Launch Profile、快照和运行时状态。
- [x] **O16**: 编排过程回放
  - 对齐说明：已基于 `automation_events + task_md_revisions` 提供结构化时间线回放，当前在 Automation 页面通过阶段历史与 task.md 历史联合查看。
- [x] **O17**: 编排模板（常见需求类型的 task.md 预设）
  - 对齐说明：已提供 `功能开发 / 缺陷修复 / 重构与验证 / 只读调研` 模板，模板可驱动默认任务提示词并进入 Launch Profile 持久化。
...
| `AutomationSettings` / `LaunchProfile` | 扩展配置 | 增加 Phase 执行器矩阵与并发策略 |
